...
\subsubsection{Methodology and SOTA Implementation}
...
A similarity threshold of 0.85 was applied to identify redundant agreements (\ref{tab:deduplication}).
...
Tables \ref{tab:profile_task2_ner_single} and \ref{tab:profile_task2_parties_mistral} show technical metrics of Task 2 implementation.
...
Table \ref{tab:profile_task3_type_bart} shows technical metrics for running BART zero-shot model used for solving Task 3.
...
The following tables summarize runtime/memory consumption and representative extracted agreement validity periods for TreeTagger-based HeidelTime validity extraction on the full evaluation corpus.

% =========================
% Table 17: Runtime & Memory
% =========================
\begin{table}[!ht]
\small
\centering
...
\bottomrule
\end{tabular}
\caption{Source of extracted end dates across agreements. Explicit end dates are directly stated in the text, while implicit end dates are inferred from temporal context.}
\label{tab:end_date_source}
\end{table}


 \\
 Table \ref{tab:end_date_source} reports the availability and source of agreement end dates identified by the system. While all agreements are processed, only a subset explicitly states an end date, with additional cases inferred via heuristic fallback rules, and the remainder lacking sufficient temporal information.

% =========================
% Table 19: Examples with evidence
% (single-column float to avoid two-column float whitespace)
% =========================
\begin{table}[!ht]
\footnotesize
\centering
...
\begin{tabularx}{\columnwidth}{p{0.18\columnwidth} p{0.11\columnwidth} p{0.11\columnwidth} p{0.10\columnwidth} X}
\toprule
\textbf{Doc} & \textbf{Start} & \textbf{End} & \textbf{Dur.} & \textbf{Evidence (excerpt)} \\
\midrule
Texas\_48 & 2019-09-27 & 2021-08-31 & -- &
``... intended to be in place until August 31, 2021.'' \\
Texas\_50 & 2023-04-27 & 2025-03-31 & -- &
``... from April 27, 2023, until March 31, 2025 ...'' \\
NewJer.\_9 & 2008-05-07 & 2010-09-30 & -- &
``... will terminate on September 30, 2010 ...'' \\
Alabama\_3 & -- & -- & 3 years &
``... shall remain in force for a period of three years.'' \\
Arkansas\_6 & 2018-01-01 & -- & 5 years &
``... in full force and effective for a period of 5 years.'' \\
\bottomrule
\end{tabularx}
\caption{Representative high-confidence validity periods with short supporting excerpts.}
\label{tab:task6_top_examples_verified}
\end{table}


Table~\ref{tab:task6_top_examples_verified} presents representative high-confidence agreement validity periods extracted by the system, including start dates, end dates, or explicitly stated durations, along with the supporting textual evidence. These examples illustrate cases where temporal validity is expressed using clear legal force language, such as effective dates, termination clauses, or fixed-term durations.

% =========================
% CRITICAL: stop Task-6 floats from drifting into Task-8
% Put this IMMEDIATELY before the next subsection.
% =========================
\FloatBarrier
...
The purpose of this task was to analyze the depth of agreements by examining word counts, with the aim of identifying which partners provide the most detailed documentation (Figure \ref{fig:txt}). The results are as follows:
\begin{itemize}
...
\begin{figure}[H]
    ...
    \caption{Top 10 partners by average agreement length}
    \label{fig:txt}
\end{figure}

We also categorized agreements into seven semantic clusters (Table \ref{tab:agreement_categories}) based on our HDBSCAN clustering examination to contextualize the verbosity.
...
Here is the breakdown of the visible categories (Figure \ref{fig:tab2}):
\begin{itemize}
    \item \textbf{Uncategorized (172)}: The significant majority of agreements did not fall into a clear semantic cluster, it usually means one of: missing text (no content to embed), very generic legal text	(“Agreement between the State of X…”), falls outside your label set	(e.g. environmental, administrative), threshold too high	(conservative assignment)
    \item \textbf{Sister Cities \& Cultural Exchange (60)}: The most frequent specific category.
    \item \textbf{Technology, Science \& Innovation (35)}: This cluster includes agreements focused on high-tech sectors, such as biotechnology, startups, digital economy, and general R\&D.
    \item \textbf{Environmental \& Climate Action (28)}: This group pertains to sustainability efforts, including renewable energy, water management, conservation, and emissions reduction.
    \item \textbf{Administrative, Legal \& Regulatory (3}): The least frequent visible category, involving formal frameworks, confidentiality, information sharing, and enforcement cooperation.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.96\linewidth]{images/TAB.png}
    \caption{Final results of document categorization}
    \label{fig:tab2}
\end{figure}
...
\section{Appendix - Shortcuts of States}
Here, we attach a table with shortcuts of states that are used in some charts.

\begin{table}[H]
\centering
\caption{Two-Letter State and Territory Abbreviations}
\label{tab:state_abbreviations}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}ll ll ll@{}}
\toprule
\textbf{State} & \textbf{Abbr.} &
\textbf{State} & \textbf{Abbr.} &
\textbf{State} & \textbf{Abbr.} \\
\midrule
...
Kansas & KS & Northern Mariana Is. & MP & Wyoming & WY \\
\bottomrule
\end{tabular}%
}
\end{table}
