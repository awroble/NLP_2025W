# Taken from https://github.com/awroble/ClickbaitDetectionThesis/blob/main/other/measure.py
import warnings
import string 
...
def common_words_ratio(text):
    ...
    return len(common_words) / (len(words)+ 1e-9)

def capital_letters_ratio(text):
    """ calculates capital letters ratio """
    text = remove_punctuation(text)
    return sum([char.isupper() for char in list(text)]) / (len(text) + 1e-9)
...
def punctuation_ratio(text):
    ...
    return sum([1 for x in text if x in string.punctuation]) / (len(text)+ 1e-9)
...
def type_token_ratio(text):
    ...
    return len(unique_words) / (len(words)+ 1e-9)

def corrected_type_token_ratio(text):
    """ lexical richness, the higher the higher diversity """
    words = word_tokenize(remove_punctuation(text.lower()))
    unique_words = set(words)
    return len(unique_words) / (np.sqrt(2 * len(words))+ 1e-9)
