from src.utils import set_seed, profile_gpu
from .base_runner import BaseRunner

class MultiTurnRunner(BaseRunner):
    """
    MultiTurn runner.
    """
    def __init__(self, models, conversations, output_dir="results"):
        super().__init__(models, prompts=[], output_dir=output_dir)
        self.conversations = conversations

    @profile_gpu
    def _run_turn(self, model, prompt, generation_params):
        """
        Method to run one turn in multiturn conversation.
        :param model: HFModel instance.
        :param prompt: Dictionary with prompt.
        :param generation_params: Model generation parameters.
        :return: Model response.
        """
        output = model.generate(prompt, **generation_params)
        return output

    def run_conversation(self, model_name, conversation, generation_params, seed):
        """
        Method to run multiturn conversation.
        :param model_name: Name of model. From 'configs/models.yaml'.
        :param conversation: Conversation instance.
        :param generation_params: Model generation parameters.
        :param seed: Random seed.
        :return: Result of the whole conversation.
        """
        set_seed(seed)
        model = self.models[model_name]
        all_turns = []
        prompt_id = conversation[0].get("id", "conversation")
        conversation_id = prompt_id.rsplit("-", 1)[0]

        for prompt in conversation:
            result = self._run_turn(model, prompt, generation_params)
            all_turns.append({
                "model_name": model_name,
                "prompt_id": prompt.get("id", prompt_id),
                "seed": seed,
                'prompt': prompt['prompt'],
                "output": result["output"],
                "metrics": result["metrics"]
            })

        self._save_result_to_file(all_turns, model_name, conversation_id, seed)

        model.clear_history()
        return (
            all_turns)

    def run_all(self, generation_params, seeds):
        """
        Method to run all conversations on all models.
        :param generation_params: Model generation parameters.
        :param seeds: Random seeds.
        :return: List of all conversations.
        """
        all_results = []
        for model_name in self.models:
            for seed in seeds:
                for conversation in self.conversations:
                    result = self.run_conversation(model_name, conversation, generation_params, seed)
                    all_results.append({
                        "model_name": model_name,
                        "seed": seed,
                        "conversation": result
                    })
        return all_results
